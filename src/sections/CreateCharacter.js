...
const CreateCharacter = (props) => {
  ...
  const getCharData = () => {
    let char = props.characterList.filter((character) => {
      ...
      case "Create":
        ...
        let charModel = {
          ...
          charPersonalityPolarOpp:
            exportObject.randomPersonalityPolarOpposite(),
          charPersonalityDescription:
            exportObject.getRandomPersonalityDescription(),
  ...
  function createJSONData(elementArr) {
    ...
    for (let i = 0; i < keys.length + charKeyMod; i++) {
      if (elementArr[i].type === "text") {
        tObj.Character[keys[i - charKeyMod]] = elementArr[i].value;
      } else {
        charKeyMod++;
      } // this is to skip all form elements that aren't text
  ...
  const splitData = () => {
    ...
    if (charData != null) {
      Object.keys(charData).map((k, i) => {
        if (i < 7) {
          parts[0][k] = charData[k];
        } else {
          parts[1][k] = charData[k];
        }
      });
    }
    return parts;
                      ...
                      Object.keys(splitData()[0]).map((oneKey, i) => {
                        return (
                          <Col md="12" lg="12" xl="12">
                            <EditFieldInput
                              genBtnVisible={
                                getGeneratorFunc(oneKey) ? true : false
                              }
                              genFunc={generatorClick}
                              changeFunc={onChangeInput}
                              saveFunc={onSaveInput}
                              value={charData[oneKey]}
                              ph={oneKey}
                              inMode={mode}
                            />
                          </Col>
            ...
            <div class="underPicText">
              ...
              {mode === "View" && charData ? (
                <Tooltip
                  ...
                  target="editBtn"
                  toggle={() => {
                    setEditTooltipOpen(!editTooltip);
                  }}
                >
                  Edit Field
                </Tooltip>
              ...
              {mode === "Edit" ? (
                <>
                  <Tooltip
                    placement="top"
                    isOpen={generateTooltip}
                    target="generateBtn"
                    toggle={() => {
                      setGenerateTooltipOpen(!generateTooltip);
                    }}
                  >
                    Generate Field
                  </Tooltip>
                  <Tooltip
                    ...
                    target="saveBtn"
                    toggle={() => {
                      setSaveTooltipOpen(!saveTooltip);
                    }}
                  >
                    Save Field
                  </Tooltip>
